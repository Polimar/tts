...
async def register(
    ...
    return await run_blocking_io(_register, "POST /auth/register")
...
async def login(body: LoginRequest, request: Request, response: Response) -> AuthResponse:
    ...
    return await run_blocking_io(_login, "POST /auth/login")
...
async def logout(
    ...
    await run_blocking_io(_logout, "POST /auth/logout")
...
async def me(user_id: str = Depends(get_current_user_id)) -> UserOut:
    ...
    return await run_blocking_io(_me, "GET /auth/me")
