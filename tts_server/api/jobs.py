...
async def create_job(body: JobCreateRequest, user_id: str = Depends(get_current_user_id)) -> JobOut:
    ...
    return await run_blocking_io(_create, "POST /jobs")


@router.get("", response_model=list[JobOut])
async def list_jobs(user_id: str = Depends(get_current_user_id)) -> list[JobOut]:
    def _list() -> list[JobOut]:
        return [_job_to_out(j) for j in get_db().list_jobs(user_id)]

    return await run_blocking_io(_list, "GET /jobs")


@router.get("/{job_id}", response_model=JobOut)
async def get_job(job_id: str, user_id: str = Depends(get_current_user_id)) -> JobOut:
    try:
    ...
    return await run_blocking_io(_get, f"GET /jobs/{job_id}")
...
async def download_job_audio(
    ...
    return await download_job(job_id, "wav", user_id)
...
async def download_job(
    ...
    def _resolve() -> tuple[Path, str, str]:
        job = get_db().get_job(job_id, user_id)
        if not job:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Job not found")
        ...
        return path, media, Path(path).name

    path, media, filename = await run_blocking_io(
        _resolve,
        f"GET /jobs/{job_id}/download/{format}",
    )
