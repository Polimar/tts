...
export function VoicesPage() {
  ...
  const handleUpload = async (e: FormEvent) => {
    ...
    setUploading(true)
    try {
      await createVoice(name.trim(), refText.trim(), file)
          ...
          {voices.map((voice) => (
            <article key={voice.id} className="voice-card">
              ...
              <dl className="voice-card__meta">
                ...
                <div>
                  <dt>Testo riferimento</dt>
                  <dd className="voice-card__ref-text">{voice.ref_text}</dd>
                </div>
                <div>
                  <dt>Creata</dt>
        ...
        <div className="dialog-overlay" role="presentation" onClick={() => setShowUpload(false)}>
          <div className="dialog" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={(e) => void handleUpload(e)}>
              ...
              <label className="field">
                <span className="field__label">Testo pronunciato nel campione</span>
                <textarea
                  ...
                  required
                  rows={3}
                  placeholder="Scrivi esattamente ciò che si sente nell'audio di riferimento…"
                />
              </label>
              <label className="field">
